...
var (
	...
	KPOWFlag = &cli.BoolFlag{
		Name:     "kpow",
		Usage:    "KPOW network: pre-configured proof-of-work main network",
		Category: flags.EthCategory,
	}
	TKPOWFlag = &cli.BoolFlag{
		Name:     "tkpow",
		Usage:    "TKPOW network: pre-configured proof-of-work test network",
		Category: flags.EthCategory,
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	if ctx.IsSet(BootnodesFlag.Name) {
		...
		switch {
		case ctx.Bool(KPOWFlag.Name):
			urls = params.KPOWBootnodes
		case ctx.Bool(TKPOWFlag.Name):
			urls = params.TKPOWBootnodes
...
func dataDirPathForCtxChainConfig(ctx *cli.Context, baseDataDirPath string) string {
	switch {
	case ctx.Bool(KPOWFlag.Name):
		return filepath.Join(baseDataDirPath, "kpow")
	case ctx.Bool(TKPOWFlag.Name):
		return filepath.Join(baseDataDirPath, "tkpow")
	case ctx.Bool(ClassicFlag.Name):
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, KPOWFlag, TKPOWFlag, MainnetFlag, DeveloperFlag, DeveloperPoWFlag, SepoliaFlag, ClassicFlag, MordorFlag, MintMeFlag, HoleskyFlag)
	...
	switch {
	case ctx.Bool(KPOWFlag.Name):
		SetDNSDiscoveryDefaults(cfg, params.KPOWGenesisHash)
	case ctx.Bool(TKPOWFlag.Name):
		SetDNSDiscoveryDefaults(cfg, params.TKPOWGenesisHash)
...
func genesisForCtxChainConfig(ctx *cli.Context) *genesisT.Genesis {
	...
	switch {
	case ctx.Bool(KPOWFlag.Name):
		genesis = params.DefaultKPOWGenesisBlock()
	case ctx.Bool(TKPOWFlag.Name):
		genesis = params.DefaultTKPOWGenesisBlock()
	case ctx.Bool(MainnetFlag.Name):
